       ...
       <div className="flex-1 flex flex-col bg-white relative min-w-0">
          {activeSession ? (
             <>
               ...
               <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scroll bg-slate-50/30 flex flex-col">
                  {activeSession.messages.map((msg, idx) => {
                     const isMe = msg.senderId === user.id;
                     const isEd = editingMessageId === msg.id;
                     return (
                        <div key={idx} className={`group flex gap-3 ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
                           <img src={isMe ? user.avatar : getPartner(activeSession).avatar} className="w-8 h-8 rounded-full bg-slate-200 object-cover mt-1"/>
                           ...
                           <div className={`flex flex-col ${isMe ? 'items-end' : 'items-start'} max-w-[75%]`}>
                              ...
                              {msg.blocks?.map((block, bIdx) => (
                                 <div key={bIdx} className="mt-2 w-full">
